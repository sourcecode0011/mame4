...
//mame4droid funtions
int  (*android_main)(int argc, char **argv,const char* game)=NULL;
...
JNIEXPORT void JNICALL Java_com_seleuco_mame4droid_Emulator_runT
  (JNIEnv *env, jclass c,jstring game){
#ifdef DEBUG
    __android_log_print(ANDROID_LOG_DEBUG, "mame4droid-jni", "runThread");
#endif
    if(android_main!=NULL){
       const char *value = (*env)->GetStringUTFChars(env, game, 0);
       android_main(0, NULL,value);  
       (*env)->ReleaseStringUTFChars(env, game, value);
    }
    else
