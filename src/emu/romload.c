...
#define	LOGX(...) __android_log_print(ANDROID_LOG_INFO, "mame4", __VA_ARGS__);
...
LOGX("%s %d \n", __func__,__LINE__);
	...
	if (romdata->errors != 0)
	{LOGX("%s %d %s \n", __func__,__LINE__,romdata->errorstring.cstr());
...
static int open_rom_file(rom_load_data *romdata, const char *regiontag, const rom_entry *romp)
{
	...
	for (drv = romdata->machine->gamedrv; romdata->file == NULL && drv != NULL; drv = driver_get_clone(drv))
		...
		{
			const char *p = options_get_number();
			char fname[128] = { 0 };
			if (strlen(p)>0)
				sprintf(fname, "%s%s"PATH_SEPARATOR"%s", p, drv->name, ROM_GETNAME(romp));
			else sprintf(fname, "%s"PATH_SEPARATOR"%s", drv->name, ROM_GETNAME(romp));
