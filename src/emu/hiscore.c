...
const char *db_filename = "/sdcard/MAME4droid/hiscore.dat"; /* high score definition file */
...
{
//	LOG("TIMER_CALLBACK hiscore_periodic =%x=%d \n",state.mem_range,state.hiscores_have_been_loaded);
	if (state.mem_range)
	{
...
void hiscore_init (running_machine *machine)
{
	...
	//LOG("hiscore_init === %d = \n",state.hiscores_have_been_loaded);
	if(state.hiscores_have_been_loaded)hiscore_save(machine);
	...
	if(filerr == FILERR_NONE)
	{
		char buffer[MAX_CONFIG_LINE_SIZE]={0};
...
//LOG("hiscore_init =3== \n");
	...
	//machine->add_notifier(MACHINE_NOTIFY_SAVEHIS, hiscore_save_input);
}
