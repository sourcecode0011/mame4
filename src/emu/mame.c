...
void stateSaveLoad(const char* file, bool isload)
{	
	if (global_machine)
	{
		...
		if (isload){
			...
			//global_machine->schedule_load(file);
		}
		else global_machine->schedule_save(file);
	}
}
/*-------------------------------------------------
...
LOG("%s %d %s  \n",__func__,__LINE__,astring_c(&gamename));
