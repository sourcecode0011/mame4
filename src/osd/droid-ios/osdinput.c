    ...
    if (mystate == STATE_NORMAL)
    {
        ...
        for (int i=0; i<4; i++)
        {
            ...
            if (joystick_read_analog(i, 'x') == 0 && joystick_read_analog(i, 'y')==0)
            {
				...
				//__android_log_print(ANDROID_LOG_DEBUG, "MAME4droid.so", " analog %d %f %f",i,mouse_x[myosd_pxasp1 ? 0 : i],mouse_y[myosd_pxasp1 ? 0 : i]);
//__android_log_print(ANDROID_LOG_DEBUG, "MAME4droid.so", "m analog %d %d %d",i,mouse_axis[i][0], mouse_axis[i][1]);
            }
            else
            {
