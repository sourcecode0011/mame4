...
float joy_analog_x[4]={0};
float joy_analog_y[4]={0};

float lightgun_x[4]={0};
float lightgun_y[4]={0};
...
int isstateloadsave=0;
char state_file[256] = {0};
//////////////////////// android
...
void(*onStateEvent_callback)(int, int, int)=NULL;
...
extern "C" void sendEventInfo(int ev,int val, int flag)
{
	if (onStateEvent_callback)
		onStateEvent_callback(ev, val, flag);
}
extern "C" void setGameEventCallbacks(void(*on_event_java)(int, int, int))
{
#ifdef ANDROID
	__android_log_print(ANDROID_LOG_DEBUG, "libMAME4droid.so", "setGameEventCallbacks %x ", on_event_java);
#endif
	onStateEvent_callback = on_event_java;
	
}
extern  void stateSaveLoad(const char* file, bool isload);
extern "C" int setStateFile(const char* file)
{
#ifdef ANDROID
	__android_log_print(ANDROID_LOG_DEBUG, "libMAME4droid.so", "setStateFile == %s ", file);
#endif
	isstateloadsave = 1;
	strcpy(state_file, file);
	//stateSaveLoad(file, false);
	return 0;
}

extern "C" int getStateFile(const char* file)
{
#ifdef ANDROID
	__android_log_print(ANDROID_LOG_DEBUG, "libMAME4droid.so", "getStateFile == %s ", file);
#endif
	isstateloadsave = 2;
	strcpy(state_file, file);
	//stateSaveLoad(file, true);
	return 0;
}
extern "C" int gameInfo_event(int ev, int vl, int fl)
{
#ifdef ANDROID
	__android_log_print(ANDROID_LOG_DEBUG, "libMAME4droid.so", "gameInfo_event == %x = %x = %x", ev, vl, fl);
#endif
	return 0;
}
...
//	__android_log_print(ANDROID_LOG_DEBUG, "libMAME4droid.so", "set_pad %ld i = %d ",pad_status, i);
}
...
void setMyValue(int key,int i, int value){
	...
	switch(key)
	{
	    ...
	    case 17:
			myosd_savestate = value; stateSaveLoad(state_file, false); break;
	    case 18:
			myosd_loadstate = value; isstateloadsave = 2; break;
	    case 20:
